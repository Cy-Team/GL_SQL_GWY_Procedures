...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			WHERE A30RKeyID NOT IN (SELECT isnull(W02A30RKeyID, '') FROM Data_DB21_W02) AND A30RKeyID IS NOT NULL 
		...
		BEGIN
			...
			LEFT JOIN Data_Person_A02 _A02 ON _A02.PersonID = _A01.PersonID AND _A02.IsLastRow = 1
			WHERE A29RKeyID NOT IN (SELECT isnull(W03A29RKeyID, '') FROM Data_DB21_W03) AND A29RKeyID IS NOT NULL 
			AND _A01.PersonID IN (SELECT PersonID FROM #WHERE_SCOPE_PERSONID)
		...
		BEGIN
			...
			AND A15RKeyID NOT IN (SELECT isnull(W04A15RKeyID, '') FROM Data_DB21_W04) AND A15RKeyID IS NOT NULL 
			AND _A01.PersonID IN (SELECT PersonID FROM #WHERE_SCOPE_PERSONID)
		...
		BEGIN
			...
			AND A11RKeyID NOT IN (SELECT isnull(W05A11RKeyID, '') FROM Data_DB21_W05) AND A11RKeyID IS NOT NULL 
			AND _A01.PersonID IN (SELECT PersonID FROM #WHERE_SCOPE_PERSONID)
		...
		BEGIN
			...
			WHERE _A14.A1407 >= (SELECT A2907 FROM Data_Person_A29 WHERE PersonID = _A14.PersonID AND IsLastRow = 1)
		 	AND isnull(_A14.A14RKeyID, '') NOT IN (SELECT isnull(W06A14RKeyID, '') FROM Data_DB21_W06) 
		 	AND _A01.PersonID IN (SELECT PersonID FROM #WHERE_SCOPE_PERSONID)
		...
		BEGIN
			...
			AND A02RKeyID NOT IN (SELECT isnull(W07A02RKeyID, '') FROM Data_DB21_W07) AND A02RKeyID IS NOT NULL 
			AND _A01.PersonID IN (SELECT PersonID FROM #WHERE_SCOPE_PERSONID)
		...
		BEGIN
			...
			AND A02RKeyID NOT IN (SELECT isnull(W08A02RKeyID, '') FROM Data_DB21_W08) AND A02RKeyID IS NOT NULL 
			AND _A01.PersonID IN (SELECT PersonID FROM #WHERE_SCOPE_PERSONID)
		...
		BEGIN
			...
			AND A02GRKeyID NOT IN (SELECT isnull(W09A02GRKeyID, '') FROM Data_DB21_W09) AND A02GRKeyID IS NOT NULL 
			AND _A01.PersonID IN (SELECT PersonID FROM #WHERE_SCOPE_PERSONID)
		...
		BEGIN
			...
			AND A02G01 >= (SELECT A2907 FROM Data_Person_A29 WHERE PersonID = _A02G.PersonID AND IsLastRow = 1)
			AND A02GRKeyID NOT IN (SELECT isnull(W0AA02GRKeyID, '') FROM Data_DB21_W0A) AND A02GRKeyID IS NOT NULL 
			AND _A01.PersonID IN (SELECT PersonID FROM #WHERE_SCOPE_PERSONID)
		...
		BEGIN
			...
			AND A02RKeyID NOT IN (SELECT isnull(W0BA02RKeyID, '') FROM Data_DB21_W0B) AND A02RKeyID IS NOT NULL 
			AND (isnull(A0292, -2) = 
				CASE 
		...
		BEGIN
			...
			LEFT JOIN Data_Person_A08 _A08 ON _A08.PersonID = _A01.PersonID AND _A08.DispOrder = (
				SELECT max(DispOrder) FROM Data_Person_A08 _A08Q WHERE PersonID = _A01.PersonID
				AND A0807 = (SELECT MAX(A0807) FROM Data_Person_A08 WHERE PersonID=_A08Q.PersonID AND A0807 < _GWYZWZJ.GWYZWZJ02))
			LEFT JOIN Data_Person_A08 _A08Q ON _A01.PersonID=_A08Q.PersonID AND _A08Q.DispOrder = (
				SELECT MAX(DispOrder) FROM Data_Person_A08 AS b WHERE b.PersonID=_A08Q.PersonID AND b.A0807 =
					(SELECT MAX(A0807) FROM Data_Person_A08 AS a WHERE a.PersonID=b.PersonID AND A0837='1'))
			...
			AND GWYZWZJ02 >= (SELECT A2907 FROM Data_Person_A29 WHERE PersonID = _A01.PersonID AND IsLastRow = 1)
			AND _A01.PersonID IN (SELECT PersonID FROM #WHERE_SCOPE_PERSONID)
